...
func TestRuntimeAllowsExactLocalhostToolOriginForLocalAcceptance(t *testing.T) {
	...
	// Docker Desktop resolves the explicit host-gateway mapping to a private
	// bridge address, so local acceptance must not require literal loopback.
	runtime := tools.NewRuntime(memory, runtimeResolver{net.ParseIP("192.168.65.2")}, runtimeDoer(func(request *http.Request) (*http.Response, error) {
	...
	runtime.SetPrivateLocalhostHosts([]string{"api.complicatedauth.localhost:38080"})
