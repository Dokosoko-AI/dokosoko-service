...
func (s *Service) requireWidgetAssistant(ctx context.Context, deploymentID, _ string) error {
	...
	if err != nil {
		return errors.New("configure and enable the support AI workload before activating the widget")
	}
	_, _, credential, err := s.aiWorkloadConfiguration(ctx, product, airuntime.WorkloadSupport)
	if err != nil {
		return errors.New("configure and enable the support AI workload before activating the widget")
	}
