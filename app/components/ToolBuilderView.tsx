      ...
      <div className="tool-builder-main">
        ...
        <fieldset className="panel tool-builder-fieldset" disabled={formLocked}>
          ...
          <div className="tool-builder-fields"><div className="tool-builder-execution-grid"><label className="auth-field" htmlFor={domID("http_method")}><span>{t("toolBuilder.method")}</span><select id={domID("http_method")} data-tool-field="http_method" value={form.http_method} onChange={(event) => {
            ...
            {apiScoped ? runtimeLock ? <div className="runtime-current-summary"><span className="settings-icon"><ShieldCheck /></span><span><strong>{runtimeConnection?.name ?? t("toolBuilder.apiServiceConnection")}</strong><small>{runtimeLock.baseURL}</small></span><span><small>{t("toolBuilder.authentication")}</small><strong>{authTypeLabel(activeAuth, t)}</strong></span><span><small>{t("toolBuilder.credential")}</small><strong>{runtimeLock.credentialPresent ? t("toolBuilder.managedInAccess") : t("toolBuilder.notRequired")}</strong></span></div> : <div className="capability-unavailable"><TriangleAlert /><span><strong>{t("toolBuilder.configureServiceAccessFirst")}</strong><small>{t("toolBuilder.anAPIToolCannotStoreItsOwnEndpointOr")}</small></span><BuilderLink path={integrationPath(apiContext!.integration.id, "access")} onNavigate={onNavigate} className="entity-back-link">{t("toolBuilder.openAccess")}</BuilderLink></div> : <>
