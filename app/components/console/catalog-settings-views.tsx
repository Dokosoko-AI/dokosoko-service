...
import {
  Bot, BookOpen, Building2, ChevronRight, Copy, Plus, RefreshCw, ScrollText, Server,
...
export function SettingsView({ aiProfiles, rootUsers, onDoctor, onNavigate }: { aiProfiles: APIAIWorkloadProfile[]; rootUsers: APIUser[]; onDoctor: () => void; onNavigate: (path: string) => void }) {
  ...
  return <><PageHeading eyebrow={t("settings.administration")} title={t("navigation.settings")} action={<Button outline onClick={onDoctor}>{t("settings.runSystemDoctor")}</Button>} /><SettingsTabs active="overview" onNavigate={onNavigate} /><div className="settings-grid"><button type="button" className="settings-button" onClick={() => onNavigate(settingsPath("tenant"))}><SettingsCard icon={<Building2 />} title={t("routes.tenantSettings")} detail={t("tenantSettings.overviewDescription")} status={t("settings.manage")} statusColor="zinc" /></button><button type="button" className="settings-button" onClick={() => onNavigate(settingsPath("ai"))}><SettingsCard icon={<Bot />} title={t("settings.aiConfiguration")} detail={t("settings.activeWorkloadsAndVersionedPrompts", { count: activeWorkloads })} status={t("settings.manage")} statusColor="zinc" /></button><button type="button" className="settings-button" onClick={() => onNavigate(settingsPath("root"))}><SettingsCard icon={<ShieldCheck />} title={t("settings.rootAccess")} detail={t("settings.mfaProtectedAdministrators", { count: activeRoots.length })} status={t("settings.secure")} /></button></div></>;
}
