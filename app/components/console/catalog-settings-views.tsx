...
export function ToolsView({ tools, integrations, connections, nativePlugins, onSetNativePluginEnabled, onNavigate }: { tools: APITool[]; integrations: APIIntegration[]; connections: APIMCPConnection[]; nativePlugins: APINativePlugin[]; onSetNativePluginEnabled: (pluginID: string, enabled: boolean) => Promise<void>; onNavigate: (path: string) => void }) {
  ...
  return <><PageHeading eyebrow="Tools" title="Tools" action={<ConsoleLink path={toolBuilderPath()} onNavigate={onNavigate} className="core-button core-button-dark"><Plus data-slot="icon" />Create HTTP tool</ConsoleLink>} /><ToolsWorkspaceTabs active="catalog" onNavigate={onNavigate} />
...
export function MCPPreviewView({ product, grants, grantStatus, available, privateEndpointEnabled, onMessage, onNavigate }: {
  ...
  return <>
    <PageHeading eyebrow="Tools" title="MCP preview" action={<Button outline disabled={!available || loading} onClick={() => { beginPreviewChange(); setRefreshKey((value) => value + 1); }}><RefreshCw data-slot="icon" className={loading ? "spin" : ""} />Refresh</Button>} />
