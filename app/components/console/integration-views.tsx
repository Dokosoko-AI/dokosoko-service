...
import { IntegrationQuickStart, type IntegrationSetupStep } from "../integrations/IntegrationQuickStart";
...
import { IntegrationPublicationDeliveryStatus } from "./integrations/publication-delivery-status";
import { IntegrationPublicationReviewDialog } from "./integrations/publication-review-dialog";
  ...
  return <>
    ...
    <div className="integration-directory-wrap">
      <DataTable label={t("navigation.apis")} className="integration-directory">
        ...
        {filteredIntegrations.map((integration) => {  return <DataTableRow key={integration.id} className="integration-directory-columns integration-directory-row">
          ...
          <span><ConsoleLink path={integrationPath(integration.id)} onNavigate={onNavigate}>{t("integrations.reviewSetup")}</ConsoleLink></span>
...
function IntegrationWorkspaceView({ integration, reviewerID, tools, activeTab, loading, revisions, publishStatus, identity, distribution, busy, onEdit, onPublish, onInspectRevision, onRuntimeChanged, onMessage, onNavigate, live }: IntegrationWorkspaceViewProps) {
  const { t } = useTranslation();
  ...
  const setupSteps: IntegrationSetupStep[] = [
    { label: t("integrations.addReviewedGuidance"), detail: t("integrations.addReviewedGuidanceDetail"), ready: Boolean(publishStatus && !publishValidationCodes.has("resources_missing")), path: integrationPath(integration.id, "documentation") },
    { label: t("integrations.reviewPublication"), detail: t("integrations.reviewPublicationDetail"), ready: Boolean(publishStatus?.ready), path: integrationPath(integration.id, "history"), onSelect: () => onPublish(integration) },
    { label: t(publishStatus?.latest_revision && !publishStatus.latest_delivery_ready ? "publicationReview.finishDelivery" : "integrations.publishGuidance"), detail: t(publishStatus?.latest_revision && !publishStatus.latest_delivery_ready ? "publicationReview.finishDeliveryHelp" : "integrations.publishGuidanceDetail"), ready: Boolean(publishStatus?.latest_delivery_ready && !publishStatus.has_changes), path: integrationPath(integration.id, "history") },
  ];
  const actionableValidations = publishStatus?.validations ?? [];
  ...
  return <>
    ...
    <PageHeading eyebrow={t("integrations.copy", { family_key: String(integration.family_key), version_key: String(integration.version_key) })} title={integration.display_name} action={<span className="heading-actions"><Button outline onClick={() => onEdit(integration)}>{t("integrations.edit")}</Button>{!publishStatus ? <span className="published-state checking"><RefreshCw />{t("integrations.checking")}</span> : canPublish ? <Button color="indigo" disabled={busy} onClick={() => onPublish(integration)}><GitBranch data-slot="icon" />{t("integrations.publish")}</Button> : !publishStatus.ready ? <Badge color="amber">{t("integrations.setupRequired")}</Badge> : !publishStatus.latest_delivery_ready ? <Badge color="amber">{t("publicationReview.deliveryPending")}</Badge> : <span className="published-state"><CheckCircle2 />{t("integrations.published")}</span>}</span>} />
    ...
    {publishStatus?.latest_revision && (!publishStatus.latest_delivery_ready || activeTab === "overview" || activeTab === "history") && <IntegrationPublicationDeliveryStatus key={integration.id} status={publishStatus} onChanged={onRuntimeChanged} />}

    {activeTab === "overview" && <IntegrationQuickStart
      lifecycle={integration.lifecycle}
      status={!publishStatus ? "checking" : publishStatus.ready ? hasChanges ? "ready" : publishStatus.latest_delivery_ready ? "published" : "delivery" : "setup"}
    ...
    {activeTab === "documentation" && <div className="integration-tab-content"><APIResourcesWorkspace reviewerID={reviewerID} key={integration.id} integration={integration} live={live} onMessage={onMessage} onNavigate={onNavigate} onChanged={onRuntimeChanged} /></div>}
    ...
    {activeTab === "tools" && <div className="integration-tab-content"><IntegrationToolsWorkspace integration={integration} tools={tools} onMessage={onMessage} onNavigate={onNavigate} onChanged={onRuntimeChanged} /><AuthorizationPolicyWorkspace integration={integration} onMessage={onMessage} onChanged={onRuntimeChanged} /></div>}
...
export function IntegrationsView({ live = true, reviewerID, integrations, analyses, tools, resourceSets, sources, identity, distribution, selectedIntegrationID, activeTab = "overview", activeResourceTab = "documentation", onAddSource, onCrawlSource, onPublishSource, onAttachPublishedSource, onGenerateSetupGuide, onChanged, onMessage, onNavigate }: IntegrationsViewProps) {
  ...
  const activeIntegrationID = useRef(selectedIntegrationID);
  useEffect(() => {
    activeIntegrationID.current = selectedIntegrationID;
    ...
    function load() {
      ...
      api.integration(integrationID).then((value) => {
        if (cancelled || request !== statusRequest.current) return;
    ...
    return () => { cancelled = true; ++requests.current; window.removeEventListener("focus", load); };
  }, [selectedIntegrationID, activeTab]);

  async function refreshSelectedIntegration(integrationID = selectedIntegrationID) {
    if (!integrationID || integrationID !== activeIntegrationID.current) return;
    const request = ++statusRequest.current;
    try {
      const value = await api.integration(integrationID);
      if (request !== statusRequest.current || integrationID !== activeIntegrationID.current) return;
      setSelectedDetail(value.integration);
      setSelectedRevisions(value.revisions);
      setSelectedPublishStatus(value.publish_status);
      setLoadedIntegrationID(integrationID);
  ...
  async function saveIntegration() {
    ...
    try {
      ...
      onMessage(editingIntegration ? t("integrations.apiUpdated") : t("integrations.apiDraftCreatedAddGuidance"));
      if (!editingIntegration) onNavigate(integrationPath(saved.id, "documentation"));
    } catch (error) {
  ...
  return <>
    ...
    >
      <div className="auth-form compact-form">
        {!editingIntegration ? <>
          ...
          <p className="developer-asset-help">{t("integrations.privateDraftDescription")}</p>
        </> : <>
    ...
    {publishCandidate && <IntegrationPublicationReviewDialog key={publishCandidate.id} integrationID={publishCandidate.id} onClose={() => setPublishCandidate(null)} onNavigate={onNavigate} onPublished={async (id) => { await onChanged(); await refreshSelectedIntegration(id); onMessage(t("integrations.apiPublishedFromTheExactPreflightCandidate")); }} />}
