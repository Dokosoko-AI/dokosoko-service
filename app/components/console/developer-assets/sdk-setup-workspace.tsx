...
import { developerAssetError, LoadingPanel, PrettyJSON, ProblemPanel, recordString } from "./developer-asset-ui";
  ...
  return <>
    ...
    {!release ? <section className="panel sdk-setup-panel"><h2>{t("sdkSetup.chooseExactRelease")}</h2>{data.releases.map((value) => <div className="sdk-version-row" key={value.id}><span><strong>{value.exact_version}</strong><small>{value.visibility} · {value.lifecycle}</small></span><Button outline onClick={() => onNavigate(sdkSetupPath({ package: pkg.id, release: value.id, api: selection.api }))}>{t("sdkSetup.openRelease")}</Button></div>)}{data.releases.length === 0 && <p>{t("sdkSetup.noReleases")}</p>}<ConsoleLink path={`${sdkSetupPath({ package: pkg.id, api: selection.api })}&advanced=1`} onNavigate={onNavigate}>{t("sdkSetup.managePackage")}</ConsoleLink></section> : <>
      {!record ? <SDKGuidanceInput key={release.id} disabled={!allowed} onSubmit={ingest} /> : <section className="panel sdk-setup-panel">
        ...
        <details><summary>{t("sdkSetup.processingDetails")}</summary><PrettyJSON value={{ candidate: record.candidate, map: record.map, symbols: record.symbols, publication: publication?.publication }} /></details>
      </section>}
      <section className="panel sdk-setup-panel"><PanelHeader title={t("sdkSetup.usedByAPIs")} description={t("sdkSetup.usageHelp")} />{data.usedBy.map(({ integration, binding }) => <div className="sdk-version-row" key={binding.id}><span><strong>{integration.display_name} · {integration.version_key}</strong><small>{t("sdkSetup.boundRelease", { version: data.releases.find((value) => value.id === binding.sdk_release_id)?.exact_version ?? t("sdkSetup.otherRelease") })} · {binding.state} · {binding.assurance}</small></span><ConsoleLink path={integrationPath(integration.id, "documentation")} onNavigate={onNavigate}>{t("sdkCatalog.returnToAPI")}</ConsoleLink></div>)}{data.usedBy.length === 0 && <p>{t("sdkCatalog.thisPackageIsNotAttachedToAnAPI")}</p>}</section>
