...
import { CheckCircle2, ChevronRight, RefreshCw, ShieldCheck, XCircle } from "lucide-react";
...
import { TaskConnectionWorkspace } from "./task-connection-workspace";
...
export function IntegrationTestWorkspace({ integration, distribution, onNavigate }: { integration: APIIntegration; distribution: Distribution | null; onNavigate: (path: string) => void }) {
  ...
  return <div className="integration-tab-content">
    <TaskConnectionWorkspace key={`${integration.id}:${integration.revision}`} integration={integration} distribution={distribution} />
    <details className="panel advanced-details"><summary>{t("integrationTest.configurationPreflight")}</summary>
