...
export function IntegrationToolsWorkspace({ integration, tools, onMessage, onNavigate }: { integration: APIIntegration; tools: APITool[]; onMessage: (message: string) => void; onNavigate: (path: string) => void }) {
  ...
  return <div className="integration-tab-content">
    ...
    {activePoints.length === 0 && !bindingsLoading && !bindingsUnavailable && <div className="capability-unavailable"><ShieldCheck /><span><strong>{t("integrationTools.createAnActiveActionPolicyFirst")}</strong><small>{t("integrationTools.everyExposedToolMustPinAnExactAuthorizationPolicy")}</small></span><a href="#api-action-policies" className="entity-back-link">Open policies below</a></div>}
