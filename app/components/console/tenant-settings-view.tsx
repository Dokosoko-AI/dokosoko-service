...
export function TenantSettingsView({ product, onSave, onNavigate }: {
  ...
  const managedFields = new Set(product.managed_fields ?? []);
  ...
  return <>
    ...
    <section className="panel tenant-settings-panel">
      ...
      {managedFields.size > 0 && <div className="notice"><span><strong>{t("managedConfiguration.title")}</strong> {t("managedConfiguration.description")}</span></div>}
      <form className="auth-form compact-form tenant-settings-form" onSubmit={(event) => void submit(event)}>
        <div className="two-fields">
          <label className="auth-field" htmlFor="tenant-name"><span>{t("tenantSettings.name")}</span><Input id="tenant-name" maxLength={120} value={name} disabled={managedFields.has("name")} invalid={Boolean(name) && !trimmedName} onChange={(event) => setName(event.target.value)} /><small>{managedFields.has("name") ? t("managedConfiguration.fieldHint") : t("tenantSettings.nameHint")}</small></label>
          <label className="auth-field" htmlFor="tenant-slug"><span>{t("tenantSettings.slug")}</span><Input id="tenant-slug" maxLength={63} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" autoCapitalize="none" autoCorrect="off" spellCheck={false} value={slug} disabled={managedFields.has("slug")} invalid={Boolean(slug) && !tenantSlugPattern.test(trimmedSlug)} onChange={(event) => setSlug(event.target.value.toLowerCase())} /><small>{managedFields.has("slug") ? t("managedConfiguration.fieldHint") : t("tenantSettings.slugHint")}</small></label>
        </div>
        <label className="auth-field" htmlFor="tenant-description"><span>{t("tenantSettings.description")}</span><Textarea id="tenant-description" rows={5} maxLength={2000} value={description} disabled={managedFields.has("description")} onChange={(event) => setDescription(event.target.value)} /><small>{managedFields.has("description") ? t("managedConfiguration.fieldHint") : t("tenantSettings.descriptionHint")} {description.length}/2000</small></label>
