...
test("uploads local knowledge files without asking for duplicate source names", async () => {
  ...
  for (const option of ["Crawl website", "Add file"]) assert.ok(source.includes(option));
  for (const kind of ["website", "upload"]) assert.match(source, new RegExp(`kind: "${kind}"`));
  assert.doesNotMatch(source, /kind: "git" as const, icon: <GitBranch \/>, label: "Git repository"/);
  assert.match(source, /className="source-kind-options documentation-source-kinds"/);
  ...
  assert.match(styles, /\.documentation-source-kinds\s*\{[^}]*grid-template-columns:\s*repeat\(2/);
